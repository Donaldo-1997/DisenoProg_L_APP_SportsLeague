    ...
    public class MatchLineupController : ControllerBase
    {
        ...
        public async Task<ActionResult<MatchLineupDto>> AddPlayerToLineup(int matchId, CreateMatchLineupDto dto)
        {
            try
            {
                ...
                // Obtenemos la alineación completa del partido para devolver el registro con detalles
                var all = await _lineupService.GetLineupByMatchAsync(matchId);
                var createdWithDetails = all.FirstOrDefault(l => l.Id == created.Id);

                return StatusCode(201, _mapper.Map<MatchLineupDto>(createdWithDetails));
            }
